    ...
    case `))
        ...
        symbol.`))
			...
			for idx, term := range clause.Bindings {
				...
				// file.template:648:12
				if idx != len(clause.Bindings)-1 {
					// file.template:648:49
					{
						_n, _err := _output.Write([]byte(`,`))
						_numWritten += int64(_n)
						if _err != nil {
