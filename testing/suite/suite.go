...
func checkSuiteSignatures[SuiteT any](tb testing.TB, suite SuiteT) {
	...
	for i := 0; i < suiteValue.NumMethod(); i++ {
		...
		if strings.HasPrefix(methodName, "Test") {
			...
			if !ok {
				tb.Fatalf(
					"Unexpected method signature for %s. Expected func(*testing.T)",
					methodName)
			}
			...
			if !ok {
				tb.Fatalf(
					"Unexpected method signature for %s. Expected func(*testing.B)",
					methodName)
			}
			...
			if strings.HasSuffix(methodName, "Suite") {
				...
				if !ok {
					tb.Fatalf(
						"Unexpected method signature for %s. Expected func(testing.TB)",
						methodName)
				}
